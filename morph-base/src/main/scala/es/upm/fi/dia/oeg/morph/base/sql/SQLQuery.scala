...
class SQLQuery extends ZQuery with IQuery {
  ...
  var exprPrefer:ZExpression = null;
  var exprDIFF:String = "";
  ...
  def this(zQuery:ZQuery, e:ZExpression) = {
    this();
  ...
  override def toString() = {
    ...
    if (exprPrefer != null){
      result += "PREFERRING " + this.exprPrefer + " "
    }
    if (exprDIFF != ""){
      result += "PARTITION BY " + this.exprDIFF.substring(0,this.exprDIFF.length()-1) + " "
    }
