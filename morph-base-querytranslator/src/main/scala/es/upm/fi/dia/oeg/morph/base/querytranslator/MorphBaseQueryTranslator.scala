...
import org.apache.jena.sparql.algebra.op.OpPreferring
...
import org.apache.jena.sparql.expr.E_AndPrefer
import org.apache.jena.sparql.expr.E_Skyline
import org.apache.jena.sparql.expr.E_PriorTo
...
abstract class MorphBaseQueryTranslator(nameGenerator: NameGenerator, alphaGenerator: MorphBaseAlphaGenerator, betaGenerator: MorphBaseBetaGenerator, condSQLGenerator: MorphBaseCondSQLGenerator, prSQLGenerator: MorphBasePRSQLGenerator)
  ...
  val functionsMap: Map[String, String] = Map(
    "E_Bound" -> "IS NOT NULL", "E_LogicalNot" -> "NOT", "E_LogicalOr" -> "OR", "E_LogicalAnd" -> "AND", "E_Regex" -> "LIKE", "E_OneOf" -> "IN", "E_AndPrefer"-> "PLUS", "E_PriorTo"-> "PRIOR TO")

  val directiveMap: Map[Int, String] = Map(
    1 -> "HIGH", -1 -> "LOW")
    ...
    val result = oldSelectItems.flatMap(oldSelectItem => {
      ...
      val resultAux = {
        if (selectItemName.equalsIgnoreCase(nameSelectVar)) {
          ...
          Some(oldSelectItem);
        } else if ((Constants.PREFIX_VAR + selectItemName).equalsIgnoreCase(nameSelectVar)) {
          Some(oldSelectItem);
        } 
        else {
          None
  ...
  def trans(op: Op): IQuery = {
    val result = {
      op match {
        ...
        case opPreferring: OpPreferring => {
          this.transPreferring(opPreferring);
        }
        case _ => {
  ...
  def transExtend(opExtend: OpExtend): IQuery = {
    ...
    for (varExpr <- opExtendExprs.keySet()) {
        ...
        val exprVarName = if (expr.getVarName() == null){
          var arguments = expr.getFunction.getArgs();
          var function_transf = "" 
          if (expr.getFunction.getFunctionIRI() != null)
            function_transf = Constants.MAP_SPARQL_CUSTOM_FUNCTIONS.get(expr.getFunction.getFunctionIRI()).get + "("
          if (arguments.size() == 1 && arguments.get(0).isFunction) {
            function_transf += arguments.get(0).getFunction.getFunctionSymbol().toString.substring(7) + "(";
            arguments = arguments.get(0).getFunction.getArgs();
          }
          else if (!(arguments.size() == 1 && arguments.get(0).isVariable))
            function_transf += expr.getFunction.getFunctionSymbol().toString().substring(7) + "(";
          
          for (i <- 0 until arguments.size()) {
            if (arguments.get(i).toString.startsWith("?")) {
               val selectItem1 = this.betaGenerator.mapping.get(expr.getVarsMentioned.head).head.head;
               function_transf = function_transf + selectItem1 + "," 
            }
            else
              function_transf = function_transf + arguments.get(i) + ","            
            if ( i == arguments.size() - 1) {
                function_transf = function_transf.substring(0, function_transf.length -1) + ")"
                if (expr.getFunction.getFunctionIRI() != null && (!(arguments.size() == 1 && arguments.get(0).isVariable))) 
                  function_transf += ")"
            }
          }
          // add the new mapping var-function
          this.betaGenerator.mapping+=(varExpr->List(new ZSelectItem(function_transf)))
          val funcTrans = function_transf + " AS \"var_" + varExpr.toString.substring(1) + "\"";
          val temporal = new ZSelectItem(funcTrans);
          subOpSQL.addSelectItems(List(temporal))
          expr.getVarsMentioned.head.toString();
        }
        else {
          expr.getVarName();
        }
  ...
  def transFilter(opFilter: OpFilter): IQuery = {
    ...
    val resultFrom = {
      if (this.optimizer != null && this.optimizer.subQueryAsView) {
        ...
        val sqlFromItem = new SQLFromItem(subOpSQL.toString(), Constants.LogicalTableType.QUERY_STRING);
        sqlFromItem.databaseType = this.databaseType;
        sqlFromItem
      }
    }
  ...
  def transPreferring(opPreferring: OpPreferring): IQuery = {
    val opPreferringSubOp = opPreferring.getSubOp();
    val subOpSQL = this.trans(opPreferringSubOp);
    val transGPSQLAlias = subOpSQL.generateAlias();
    val subOpSelectItems = subOpSQL.getSelectItems().toList;
    ...
    val resultFrom = {
      if (this.optimizer != null && this.optimizer.subQueryAsView) {
        val conn = this.connection;
        val subQueryViewName = "sqf" + Math.abs(opPreferringSubOp.hashCode());
        val dropViewSQL = "DROP VIEW IF EXISTS " + subQueryViewName;
        logger.info(dropViewSQL + ";\n");
        DBUtility.execute(conn, dropViewSQL, 0);
        val createViewSQL = "CREATE VIEW " + subQueryViewName + " AS " + subOpSQL;
    ...
    val exprList = opPreferring.getExprs();
    val exprListSQL = this.transPreferring(
      exprList,
      subOpSelectItems,
      subOpSQL.getAlias());
    val transPreferringSQL = {
      if (this.optimizer != null && this.optimizer.subQueryElimination) {
        //subOpSQL.pushFilterDown(exprListSQL);
        subOpSQL.addPrefer(exprListSQL._1);
        if (exprListSQL._2 != null) subOpSQL.addDIFF(exprListSQL._2)
        subOpSQL;
      } else {
        val oldSelectItems = subOpSQL.getSelectItems();
        val newSelectItems = oldSelectItems.map(oldSelectItem => {
        ...
        val resultAux = new SQLQuery(subOpSQL);
        resultAux.setSelectItems(newSelectItems);
        resultAux.addPrefer(exprListSQL._1);
        if (exprListSQL._2 != null) subOpSQL.addDIFF(exprListSQL._2)
        //resultAux.addWhere(exprListSQL);
        resultAux;
      }
    }
    return transPreferringSQL;
  }
  ...
  def transTP(tp: Triple): IQuery = {
    ...
    if (tpSubject.isVariable()) {
      this.betaGenerator.mapping+=(tpSubject->List(new ZSelectItem("var_"+tpSubject.toString.substring(1))))
    }
  ...
  def transTP(tp: Triple, cm: MorphBaseClassMapping, predicateURI: String, unboundedPredicate: Boolean): MorphTransTPResult = {
  ...
  def transExpr(
   ...
   def transExpr(
    //op:Op,
    expr: Expr,
    subOpSelectItems: Map[Node, List[ZSelectItem]]): List[ZExp] = {
    val result: List[ZExp] = {
      if (expr.isVariable()) {
        logger.debug("expr is var");
        val exprVar = expr.asVar();
        this.transVar(exprVar, subOpSelectItems);
      } else if (expr.isConstant()) {
        logger.debug("expr is constant");
        val nodeValue = expr.getConstant();
        this.transConstant(nodeValue);
        ...
        val exprFunction = expr.getFunction();
        //List(this.transFunction(op, exprFunction, subOpSelectItems, prefix));
        List(this.transFunction(exprFunction, subOpSelectItems));
      } else {
        Nil
      }
    }
  ...
  def transFunction(
      ...
      val arguments = transExpr(func.getArgs.head, this.betaGenerator.mapping).head.toString()
      val dim = arguments
      dim
  }
  
  def transProperty(
      zExpr : List[ZSelectItem]
      ) : String = {
    
    val item = MorphSQLUtility.getValueWithoutAlias(zExpr.head)
    item
  }
  ...
  def transIF(
    ...
    val item = zExpr.getArgs
    val cond = item.head
    val translatedCond = transExpr(cond, this.betaGenerator.mapping)
    
    var dim = "CASE WHEN " + translatedCond.head.toString() + " THEN " 
    var bodyIF = ""
 
    for (i <- 1 until item.length) {
      if (i>1) bodyIF += " ELSE "
      val translatedArg = if (item(i).isInstanceOf[ExprFunction1]) {
        bodyIF += transExpr(item(i), this.betaGenerator.mapping).head
      } else if (item(i).isInstanceOf[E_Conditional]) {
        bodyIF += transIF(item(i).asInstanceOf[E_Conditional])
      }
      else {
        val aux = transExpr(item(i), this.betaGenerator.mapping).head
...
def transPreferring(
    expr: Expr,
    subOpSelectItems: List[ZSelectItem],
    prefix: String): (List[ZExp],String) = {
    var result: List[ZExp] = Nil;
    var resultDIFF:String = "";
    
    // DIFF translated
    if (expr.isInstanceOf[E_Skyline] && expr.asInstanceOf[E_Skyline].getDirective == 0) {
       val expresion = expr.asInstanceOf[E_Skyline]
       var dim:String = null
      dim = if (expresion.getVar == null) { //the dimension is a function 
        val exprFunction = expresion.getDimension.asInstanceOf[ExprFunction1]
        transFunction(exprFunction, this.betaGenerator.mapping).toString()
      } else {
        val argTranslated = this.betaGenerator.mapping.get(expresion.getVar)
        val operand = argTranslated.head
        MorphSQLUtility.getValueWithoutAlias(operand.head);
      }
      resultDIFF = dim + ","
    }
    else result = {
      if (expr.isInstanceOf[E_Skyline]) {
          val expresion = expr.asInstanceOf[E_Skyline]
          val res = if (expresion.getDirective != 0) {
              val directiveSymbol: String = directiveMap(expresion.getDirective)
              if (expresion.getVar == null) { //the dimension is a function or conditional
                if (expresion.getDimension.isInstanceOf[E_Conditional]) {//the dimension is conditional
                  val exprFunction = expresion.getDimension.asInstanceOf[E_Conditional]
                  val dim = transIF(exprFunction)
                  val zExp = new ZConstant(dim, ZConstant.COLUMNNAME);
                  val resultAux = new ZExpression(directiveSymbol);
                  resultAux.addOperand(zExp);
                  val resultAuxs  = resultAux  :: List.empty[ZExp]
                  resultAuxs
                } else {//the dimension is a function
                  val exprFunction = expresion.getDimension.asInstanceOf[ExprFunction1]
                  val dim = transFunction(exprFunction, this.betaGenerator.mapping).toString()
                  val zExp = new ZConstant(dim, ZConstant.COLUMNNAME);
                  val resultAux = new ZExpression(directiveSymbol);
                  resultAux.addOperand(zExp);
                  val resultAuxs  = resultAux  :: List.empty[ZExp]
              ...
              } else {//the dimension is an attribute
                  val argTranslated = this.betaGenerator.mapping.get(expresion.getVar)
                  //when an alias in the select is defined for a function
                  val resultAuxs = if (argTranslated == None){
                    var i = 0;
                    var position = -1;
                    var isnotFound = true;
                    val item = expresion.getVar.toString.substring(1)
                    var element = ""
                    //loop for finding the function in the select clause
                    while(i < subOpSelectItems.size && isnotFound) {
                      position = subOpSelectItems.get(i).toString.indexOf(" AS \"\"" + item + "\"");
                      if (position != -1) {
                        element = subOpSelectItems.get(i).toString.substring(0,position)
                        isnotFound = false
                      }
                      i = i + 1;
                    }  
                      
                    val zExp = new ZConstant(element, ZConstant.COLUMNNAME);
                    val resultAux = new ZExpression(directiveSymbol);
                    resultAux.addOperand(zExp)
                    Option(resultAux);
                  }
                  else   
                    argTranslated.map(operand => {
                    val item = MorphSQLUtility.getValueWithoutAlias(operand.head);
                    val zExp = new ZConstant(item, ZConstant.COLUMNNAME);
                    val resultAux = new ZExpression(directiveSymbol);
                    resultAux.addOperand(zExp);
                    resultAux; })
                  resultAuxs.toList;
          ...
          res.asInstanceOf[List[ZExp]]
      } else if (expr.isInstanceOf[E_AndPrefer]) {
          val expresion = expr.asInstanceOf[E_AndPrefer]
          val exprLeft = transPreferring(expresion.getArg1(), subOpSelectItems, prefix)
          val exprRight = transPreferring(expresion.getArg2(), subOpSelectItems, prefix)
          val exprAux = if (exprLeft._1 != Nil && exprRight._1 != Nil ) {
            val functionSymbol: String = functionsMap("E_AndPrefer");
            val resultAux = new ZExpression(functionSymbol);
            resultAux.addOperand(exprLeft._1.head);  
            resultAux.addOperand(exprRight._1.head);
            val resultAuxs = resultAux :: List.empty[ZExp]
            MorphSQLUtility.combineExpresions(resultAuxs, Constants.SQL_LOGICAL_OPERATOR_PLUS);
            resultAuxs
          } else if (exprLeft._1 == Nil && exprRight._1 != Nil ) 
            exprRight._1.head :: List.empty[ZExpression]
          else if (exprLeft._1 != Nil && exprRight._1 == Nil ) 
            exprLeft._1.head :: List.empty[ZExpression]
          else if (exprLeft._1 == Nil && exprRight._1 == Nil ) 
            List.empty[ZExp]
          if (exprLeft._2 != null)
            resultDIFF += exprLeft._2  
          if (exprRight._2 != null)
            resultDIFF += exprRight._2  
          exprAux.asInstanceOf[List[ZExp]]             
          } 
      else if (expr.isInstanceOf[E_PriorTo]) {
          val expresion = expr.asInstanceOf[E_PriorTo]
          val exprLeft = transPreferring(expresion.getArg1(), subOpSelectItems, prefix)
          val exprRight = transPreferring(expresion.getArg2(), subOpSelectItems, prefix)
          val functionSymbol: String = functionsMap("E_PriorTo");
          val resultAux = new ZExpression(functionSymbol);
          resultAux.addOperand(exprLeft._1.head);  
          resultAux.addOperand(exprRight._1.head);
          val resultAuxs  = resultAux  :: List.empty[ZExpression]
          MorphSQLUtility.combineExpresions(resultAuxs, Constants.SQL_LOGICAL_OPERATOR_PRIORTO);
          resultAuxs
      } 
      else {
        //Nil
        transExpr(expr, this.betaGenerator.mapping)
      }
    }
    (result,resultDIFF);
  }
  ...
  def transPreferring(
    exprList: ExprList,
    subOpSelectItems: List[ZSelectItem],
    prefix: String): (ZExpression,String) = {
    val exprs = exprList.getList();
    //get the expression of preferring clause
    val expr = exprs.head
    val resultAux = transPreferring(expr, subOpSelectItems, prefix)
    val result = MorphSQLUtility.combineExpresions(resultAux._1.toList, Constants.SQL_LOGICAL_OPERATOR_AND);
    (result,resultAux._2);
  }
  ...
  def transFunction(
    ...
    subOpSelectItems: Map[Node, List[ZSelectItem]]): ZExp = {
    ...
    val result: ZExp = {
      exprFunction match {
        case _: ExprFunction1 => {
          ...
          var functionSymbol: String = exprFunction match {
            case eBound: E_Bound => {
          ...
          if (functionSymbol == null ) functionSymbol = exprFunction.getFunctionSymbol.getSymbol
          val argTranslated = this.transExpr(arg, subOpSelectItems);
        ...
        case _: ExprFunction2 => {
          ...
          val leftExprTranslated = this.transExpr(leftArg, subOpSelectItems);
          val rightExprTranslated = this.transExpr(rightArg, subOpSelectItems);
          exprFunction match {
            case _: E_NotEquals => {
              val functionSymbol = {
        ...
        case eFunction: E_Function => {
          ...
          if (exprs != null && exprs.size() == 1) {
            ...
            val resultAuxs = this.transExpr(expr, subOpSelectItems);
        ...
        case _ => {
          val transArgs = for (i <- 0 until args.size()) yield {
            val arg = args.get(i);
            val zExps = this.transExpr(arg, subOpSelectItems);
        ...
        case _:ExprFunction2 => {
          ...
          exprFunction match {
            ...
            case _=> {
              ...
              val ExprTranslatedSize = if (leftExprTranslated.size() != rightExprTranslated.size()) 
                min(leftExprTranslated.size(),rightExprTranslated.size())
              else 
                leftExprTranslated.size()
              //val resultAuxs = for(i <- 0 until leftExprTranslated.size()) yield {
              val resultAuxs = for(i <- 0 until ExprTranslatedSize) yield {
                val resultAux = new ZExpression(functionSymbol);
                var leftOperand = leftExprTranslated.get(i);
                if (leftOperand.toString().endsWith("_0"))
                  leftOperand = leftExprTranslated.get(i+1);
                resultAux.addOperand(leftOperand);
                var rightOperand = rightExprTranslated.get(i);
                if (rightOperand.toString().endsWith("_0"))
                  rightOperand = rightExprTranslated.get(i+1);
                resultAux.addOperand(rightOperand);
                resultAux;
              }
          ...
          if (termCColumns1.size() == termCColumns2.size()) {
            ...
            val expsAux = for (termCColumn1 <- termCColumns1) yield {
              ...
              val gp1TermC = MorphSQLConstant.apply(
                enclosedCharacter + transGP1Alias + enclosedCharacter + "." + enclosedCharacter + termCColumn1 + enclosedCharacter, ZConstant.COLUMNNAME, this.databaseType, null);
              val gp2TermC = MorphSQLConstant.apply(
                enclosedCharacter + transGP2Alias + enclosedCharacter + "." + enclosedCharacter + termCColumn2 + enclosedCharacter, ZConstant.COLUMNNAME, this.databaseType, null);
 ...
 def transVar(theVar: Var, subOpSelectItems: Map[Node, List[ZSelectItem]]): List[ZExp] = {
    val varTranslated = this.betaGenerator.mapping.get(theVar)
    val resultAuxs = varTranslated.map(operand => {
      val item = MorphSQLUtility.getValueWithoutAlias(operand.head);
      val zExp = new ZConstant(item, ZConstant.COLUMNNAME);
           zExp ;
            })
...
/*            
  ...
  override def translateUpdate(stg: OpBGP) = {
    ...
    if (!isSTG) {
      val errorMessage = "Only STG pattern is supported for update o}peration!"
      logger.error(errorMessage);
      throw new Exception(errorMessage);
    }
  ...
  def initializeMapVarIsNullable(op: Op): Unit = {
    op match {
      ...
      case opPreferring: OpPreferring => { //FILTER pattern
        val exprs = opPreferring.getExprs();
        val subOp = opPreferring.getSubOp();
        val subOpRewritten = this.initializeMapVarIsNullable(subOp);
      }
      case opProject: OpProject => {
