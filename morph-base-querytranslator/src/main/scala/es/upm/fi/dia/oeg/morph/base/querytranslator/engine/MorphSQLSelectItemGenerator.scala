			  ...
			  val oldSelectItemsIterator = oldSelectItems.iterator();
				while(oldSelectItemsIterator.hasNext()) {
					...
					if(oldAlias == null || oldAlias.equals("")) {
						...
						//var isFound = false
  						Constants.MAP_SPARQL_CUSTOM_FUNCTIONS.values.foreach(
  						    c => if (oldSelectItem.toString().toUpperCase().startsWith(c.toString()))
  						      isFound = true) 
  						if (isFound) {
  						  oldTable == null
  						  selectItemName = oldSelectItem.toString
						}
					} else {
					...
					if(selectItemName.equalsIgnoreCase(nameSelectVar)) {
					  ...
					  val position = selectItemName.indexOf(" AS ")
  					if (position != -1) {
  					  newSelectItemAlias = selectItemName.substring(position+4)
  					  newSelectItemAlias = newSelectItemAlias.replaceAll("var_", "");
  					  selectItemName = selectItemName.substring(0,position)
  					  newSelectItem = new ZSelectItem(selectItemName);
						  newSelectItem2 = MorphSQLSelectItem(selectItemName, null, this.dbType, null);  
  					}
					} else {
					  newSelectItem = null;
					}
